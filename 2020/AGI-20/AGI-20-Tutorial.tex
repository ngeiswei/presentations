...
\begin{frame}
  ...
  %% OK, but why is that called simple truth value? Well, you guess
  %% right it's because we also have more complex values, and I'm not
  %% gonna get into that but I just want to mention that underneath
  %% a truth value is a second order probability, it's a probability
...
\end{block}
  ...
  \textit{Truth Value} = Second order distribution
...
\begin{frame}
  ...
  %% distribution is flat (well, there are other possibilities but that's
  %% the simplest one to explain), and as measure as the number of
  %% observations grow the second order distribution narrows down to a
  %% particular value.
\end{frame}

\begin{frame}
  OK, so now let's show simple reasoning, A->B, B->C |- A->C
