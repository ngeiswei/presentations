...
\section{Probability of Conjecture to be
  Theorem (in PLN)}

\begin{frame}
  \emph{\underline{State of the Art (notable papers):}}\\[0.5cm]
  \begin{itemize}
  \item \emph{Logical Prior Probability}, Abram Demski (2016)
  \item \emph{Uniform Coherence}, Scott Garrabrant et al (2016)
  \item \emph{Logical Induction}, Scott Garrabrant et al (2016)
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Ternary predicate relating theories, proofs and propositions}
  %% \emph{\underline{Ternary predicate relating theories, proofs and propositions:}}\\[1cm]
  $$\Theta : {\color{violet}\TheoryT} \times {\color{blue}\ProofT}
  \times {\color{red}\PropositionT} \to \BoolT$$

  \begin{center}
  \digraph[scale=0.8]{thetapln}{
    rankdir=LR;
    node[shape=none, label="𝛩"] theta;
    node[shape=none, label="Predictive Patterns"] PP;
    node[shape=none, label="Estimate Conjectures"] EC;
    theta -> theta [label="PLN"];
    theta -> PP [style=dashed];
    PP -> EC [style=dashed];
  }
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \item Composite Predicates: $\text{Bird} \land \neg \text{Penguin}\ \limp\ \text{Fly}$
...
\begin{frame}
  ...
  \underline{\emph{Uncertain Reasoning:}}
  $\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ H \limp \forall x\ P(x)\ \ ?$\\[0.1cm]
  \pause
  \begin{columns}
    \column{7cm}
    \only<1->{
...
\begin{frame}
  ...
  \begin{center}
  \underline{\emph{Induction + Abduction + Revision:}}\\
    \digraph[scale=0.4]{inductionabductiontheo}{
      rankdir=LR;
      node[shape=none, label="T₁"] T1;
      node[shape=none, label="T₂"] T2;
      node[shape=none, label="T₃"] T3;
...
\begin{frame}[fragile]
  ...
  & &
  {\color{blue}
    (\lambda\ \texttt{mp2.1}\ \texttt{mp2.3}\ (\axmp\ \texttt{mp2.1}\ \texttt{mp2.3}))
    },
  \\
  & &
  {\color{red}
...
\begin{frame}[fragile]
  ...
  & &
  {\color{blue}
    (\lambda\ \texttt{mp2.1}\ \texttt{mp2.3}\ (\axmp\ \texttt{mp2.3}\ \texttt{mp2.1}))
    },
  \\
  & &
  {\color{red}
...
\begin{frame}
  \frametitle{PLN and Theorem Proving}
  \emph{\underline{Crisp Patterns:}}\\[0.5cm]
  \begin{itemize}
  \item Modus ponens:
    $$\Theta({\color{violet}\Gamma}, {\color{blue}f}, {\color{red}a \to b}) \land \Theta({\color{violet}\Gamma}, {\color{blue}x}, {\color{red}a}) \limp
    \Theta({\color{violet}\Gamma}, {\color{blue}f(x)}, {\color{red}b})$$
    $$\measeq$$
    $$\STV{1}{1}$$
  \item Existential Quantification Introduction:
    $$\Theta({\color{violet}\Gamma}, {\color{blue}\Pi},
    {\color{red}\Tau}) \land (\closed\ {\color{violet}\Gamma}) \land
    (\closed\ {\color{blue}\Pi}) \land (\closed\ {\color{red}\Tau})
    \limp \exists {\color{blue}\pi}\ \Theta({\color{violet}\Gamma},
          {\color{blue}\pi}, {\color{red}\Tau})$$
    $$\measeq$$
    $$\STV{1}{1}$$
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{PLN and Theorem Proving}
  \emph{\underline{Uncertain Patterns:}}\\[0.5cm]
  \begin{itemize}
  \item Marginal estimate: $$\exists
    {\color{blue}\pi}\ \Theta({\color{violet}\text{PC}},
    {\color{blue}\pi}, {\color{red}\tau})$$
    $$\measeq$$
    $$\STV{0.001}{0.8}$$
  \item Conditional estimate: $$P({\color{red}\tau}) \limp \exists
    {\color{blue}\pi}\ \Theta({\color{violet}\text{PC}},
    {\color{blue}\pi}, {\color{red}\tau})$$
    $$\measeq$$
    $$\STV{0.2}{0.7}$$
...
\section{Use such Estimates to Guide Reasoning}

\begin{frame}
  \frametitle{Estimate Probability of Conjecture to be Theorem in
    Practice}
  \begin{center}
    {\footnotesize
      \MeTTa{(bc PARAMS THEORY (: $proof PROP))}\\[0.6cm]
      \pause
      $\downarrow\ \ \ \texttt{[\$proof]} = \texttt{z}$\\[0.6cm]
      \MeTTa{(bc PLN_PARAMS PLN_THEORY (: $pln_proof
        (}$\measeq$
      \MeTTa{(}$\exists$\MeTTa{ z (}$\Theta$
      \MeTTa{[THEORY] z [PROP])) $TV))}\\[0.6cm]
      \pause
      $\downarrow$\\[0.6cm]
      \MeTTa{$TV} = ?\\[0.2cm]
      \pause
      \begin{itemize}
      \item<+-> Complete ignorance: \MeTTa{$TV} = $\STV{1}{0}$
      \item<+-> Complete certainty: \MeTTa{$TV} = $\STV{1}{1}$
      \item<+-> Partial certainty: \MeTTa{$TV} = $\STV{0.7}{0.8}$
      \end{itemize}
    }
  \end{center}
\end{frame}
...
\begin{frame}
  \frametitle{Estimate Probability of Conjecture to be Theorem as Guide}
  \begin{center}
    \only<1>{
    \digraph[scale=0.55]{bc}{
      rankdir=TB;
      node[shape=none, label="(bc PARAMS THEORY (: $proof PROP))"] Q;
      node[shape=none, label="(: $f (-> $h PROP)), (: $x $h)"] MP;
      node[shape=none, label="PROP ≐ (-> $h $c), (: $y $c)"] LA;
      node[shape=none, label="(: Aᵢ Tᵢ) ∈ THEORY"] Ai;
      node[shape=none, label="$TV = ?"] TVMP;
      node[shape=none, label="$TV = ?"] TVLA;
      node[shape=none, label="$TV = <1, 1>"] TVAi;
      Q -> MP [label="CD"];
      Q -> LA [label="CG"];
      Q -> Ai [label="Axiom"];
      MP -> TVMP [arrowhead=none, label=""];
      LA -> TVLA [arrowhead=none, label=""];
      Ai -> TVAi [arrowhead=none, label=""];
    }%
    }%
    \only<2,4->{
    \digraph[scale=0.55]{bcred}{
      rankdir=TB;
      node[shape=none, label="(bc PARAMS THEORY (: $proof PROP))"] Q;
      node[shape=none, label="(: $f (-> $h PROP)), (: $x $h)"] MP;
      node[shape=none, label="PROP ≐ (-> $h $c), (: $y $c)"] LA;
    ...
    \only<3>{\includegraphics[scale=0.2]{figs/BC-CD-HL.png}}%
    \visible<5->{
      \only<-5>{
        {\footnotesize
          %% \emph{\underline{CD:}}
          \begin{tabular}{lll}
            \MeTTa{(bc} & \MeTTa{PLN_PARAMS PLN_THEORY} & \\
            & \MeTTa{(: $pln_proof
              (}$\measeq$
            \MeTTa{(}$\exists$\MeTTa{ f h x (}$\land$ &
            \MeTTa{(}$\Theta$
            \MeTTa{[THEORY] f (-> h [PROP]))} \\
            & & \MeTTa{(}$\Theta$ \MeTTa{[THEORY] x h))
              $TV)))}
          \end{tabular}
        }
      }%
      \only<6>{
        \vspace{0.1cm}
        {\footnotesize
          \MeTTa{(bc PLN_PARAMS PLN_THEORY (: $pln_proof (}$\measeq$
          \MeTTa{(}$\exists$\MeTTa{x} \MeTTa{(}$\Theta$
          \MeTTa{[THEORY] x H)) $TV)))}
        }
        \vspace{0.56cm}
      }%
...
\begin{frame}
  ...
  \begin{itemize}
  \item Early experiment {\small (\href{https://github.com/trueagi-io/chaining/tree/main/experimental/pln-inf-ctl}{\color{violet}{https://github.com/trueagi-io/chaining/tree/main/experimental/pln-inf-ctl}})}
    \begin{itemize}
    \item Small MetaMath corpus
    \item $1^{\text{th}}$ run: exhaustive search, \emph{populate $\Theta$}\\
      \includegraphics[scale=0.3]{figs/theta.metta.png}
    \item $2^{\text{nd}}$ run: speed-up via \emph{PLN existential reasoning}
    \item Glorified Memoizer
    \end{itemize}
  \item Future work
    \begin{itemize}
    \item Large MetaMath corpus
    \item Induction, abduction and more
    \end{itemize}
  \end{itemize}
\end{frame}
