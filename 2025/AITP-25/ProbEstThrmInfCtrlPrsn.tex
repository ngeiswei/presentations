...
\begin{frame}
  ...
  \begin{center}
    %% \digraph[scale=0.43]{bc}{
    %%   rankdir=TB;
    %%   node[shape=none, label="(bc PARAMS THEORY (: $proof PROP))"] Q;
    %%   node[shape=none, label="$proof ≐ ($f $x), (: $f (-> $h PROP)), (: $x $h)"] MP;
    %%   node[shape=none, label="PROP ≐ (-> $h $c), THEORY ∪ (: $x $h), (: $y $c)"] LA;
    %%   node[shape=none, label="(: Aᵢ Pᵢ) ∈ THEORY"] Ai;
    %%   Q -> MP [label="CD"];
    %%   Q -> LA [label="CG"];
    %%   Q -> Ai [label="Axiom"];
    %% }
