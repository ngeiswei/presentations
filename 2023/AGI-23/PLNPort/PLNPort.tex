...
\section{Download and run PLN docker image:}
\subsection{\texttt{docker run --rm -ti trueagi/pln}}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    % <BEGIN-SPEECH>
    % So, let's say we know we want to start from these premises, then
    % we can expand forward.
    % <END-SPEECH>

    \underline{Premises}: $P$, $P\rightarrow Q$, $Q\rightarrow R$\\

    % <BEGIN-SPEECH>
    % Here I'm giving away the structure of the tree but it's just for
    % convenience, it doesn't have to be predetermined.
    % <END-SPEECH>
    ...
    % <BEGIN-SPEECH>
    % And it does not have stop there, it can continue as long as
    % possible.
    % <END-SPEECH>
...
\begin{frame}
  ...
  % <BEGIN-SPEECH>
  % But that's not the only forms of chaining
  % <END-SPEECH>
...
\begin{frame}
  ...
  % <BEGIN-SPEECH>
  % We have for instance what we could call Inward Chaining.  So we
  % know from where we're starting, and we know where we want to end,
  % and we just want to find what's in the middle.
  % <END-SPEECH>
...
\begin{frame}
  ...
  % <BEGIN-SPEECH>
  % We have also what we could call Outward Chaining.  So we know
  % we're gonna use something in the middle, but we don't necessarily
  % know all the premises we're gonna need or the conclusion we want
  % to reach, and so we expand outward.
  % <END-SPEECH>
  ...
  % <BEGIN-SPEECH>
  % But even that doesn't cover all cases
  % <END-SPEECH>
...
\begin{frame}
  ...
  % <BEGIN-SPEECH>
  % Sometimes we may need what could be called Omniward Chaining.
  % Sorry for the neologism, if you know a better word I'm interested.
  % <END-SPEECH>
  ...
  \begin{columns}
    ...
    % <BEGIN-SPEECH>
    % And so the idea is that chaining may expand in all directions.
    % <END-SPEECH>
