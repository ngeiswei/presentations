...
\begin{frame}
  ...
  \begin{columns}
    ...
    % <BEGIN-SPEECH>
    % The Lag operator is a temporal transposition from the left to the
    % right.  One might say that it brings the past into the present,
    % meaning it gives you a lens to look at the past from the viewpoint
    % of the present.

    % The Lead is the opposite, that is brings the future into the
    % present.
    % <END-SPEECH>

    \underline{Temporal Predicates and Connectors}:
    \begin{itemize}
    \item<+-> $P, Q, R, \hdots: \textit{Domain} \times \TTime \mapsto
      \{\TTrue,\TFalse\}$
    \item<+-> $\llag{P}{T} \eqdef \lambda x, t. P(x, t-T)$
    \item<+-> $\llead{P}{T} \eqdef \lambda x, t. P(x, t+T)$
    \item<+-> $P \lseqand{T} Q \eqdef P \land \llead{Q}{T}$
    \item<+-> $P\ \lbseqand{T} Q \eqdef \llag{P}{T} \land Q$
    \item<+-> $P \lpreimp{T} Q \eqdef P \limp \llead{Q}{T}$
    \end{itemize}

    \pause

    \underline{Temporal Inference Rules}:

    \pause

    \begin{itemize}
    % \item<+-> Predictive Implication Direct Introduction (PIDI):\\
    %   {\small
    %     \begin{prooftree}
    %       \AxiomC{$P(a_1, t_1) \measeq \TTVPo, Q(a_1, t_1) \measeq
    %         \TTVQo, \dots, P(a_n, t_1) \measeq \TTVPn, Q(a_n, t_n) \measeq \TTVPn$}
    %       \RightLabel{(PIDI)}
    %       \UnaryInfC{$P \lpreimp{T} Q \measeq \TTV$}
    %     \end{prooftree}}
    \item<+-> Temporal Deduction (TD):
      {\small
        \begin{prooftree}
          ...
          \AxiomC{$R \measeq \TTVR$}
          \RightLabel{(TD)}
          \QuinaryInfC{$P \lpreimp{T_1+T_2} R \measeq \phi_{TD}\left(\TTVPQ, \ldots, \TTVR\right)$}
        \end{prooftree}
      }
...
\begin{frame}
  ...
  {\tiny
    \begin{prooftree}
      ...
      \RightLabel{(D)}
      \QuinaryInfC{$P \limp \llead{R}{T_1+T_2} \measeq \phi_D\left(\TTVPQ,
          \dots, \TTVR\right)$}
      \RightLabel{(IP)}
      \UnaryInfC{$P \lpreimp{T_1+T_2} R \measeq  \phi_D\left(\TTVPQ,
          \dots, \TTVR\right)$}
    \end{prooftree}}
...
\begin{frame}
  \frametitle{Procedural Reasoning}

  \underline{Procedural Construct}:
  \begin{itemize}
  \item Cognitive Schematic
    $$\textit{Context} \land \textit{Action} \lpreimp{T} \textit{Goal} \measeq \textit{TV}$$
  \end{itemize}

  \pause

  \underline{Procedural Inference Rule}:
  \begin{itemize}
  \item Procedural Deduction (PD)
    {\small
      \begin{prooftree}
        ...
        \TrinaryInfC{$C_1 \land A_1 \lseqand{T_1} A_2 \lpreimp{T_1+T_2}
          C_3 \measeq \phi_{PD}\left(\TTV^{\ \!12}, \TTV^{\ \!23}, \dots\right)$}
      \end{prooftree}}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  {\tiny
    \begin{prooftree}
      ...
      \RightLabel{(PD)}
      \QuinaryInfC{$C_1\!\land\! A_1\!\lseqand{T_1}\!A_2
        \lpreimp{T_1+T_2} C_3 \measeq \phi_{PD}(\TTV^{\ \!12}, \TTV^{\
          \!23}, \TTV^{\ \!1}, \TTV^{\ \!2}, \TTV^{\ \!3})$}
    \end{prooftree}}
  ...
  {\tiny
    \begin{prooftree}
      ...
      \RightLabel{(IP)}
      \UnaryInfC{$C_1\!\land\! A_1\!\lseqand{T_1}\!A_2
        \lpreimp{T_1+T_2} C_3 \measeq \phi_{PD}\left(\TTV^{\ \!2}, \TTV^{\
          \!23}, \TTV^{\ \!1}, \TTV^{\ \!2}, \TTV^{\ \!3}\right)$}
    \end{prooftree}}
